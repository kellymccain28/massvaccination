...
outcomes_averted <- function(df, byyear = FALSE){
  ...
  if(byyear == FALSE){
    df <- df |>
      ungroup() |>
      group_by(age_lower, age_upper, ID, int_ID)
    
    joinvars <- c('ID', 'drawID', 'age_lower','age_upper', 'MDA')
    
    baseline_vars <- c('file', 'ID', 'scenario', 'drawID', 'age_lower', 'age_upper', 'MDA','daly_baseline', 'cases_baseline',
                       'severe_baseline', 'deaths_baseline', 
                       # 'u5_dalys_baseline','u5_cases_baseline', 'u5_severe_baseline', 
                       'dose3')
  } else if(byyear == TRUE){
    df <- df |> 
      ungroup()|>
      group_by(age_lower, age_upper, t, ID, int_ID)
    
    joinvars <- c('ID', 'drawID', 'age_grp', 't', 'MDA')
    
    baseline_vars <- c('file', 'ID', 'scenario', 'drawID', 'age_grp', 'MDA','daly_baseline', 'cases_baseline',
                       'severe_baseline', 'deaths_baseline', 
                       # 'u5_dalys_baseline','u5_cases_baseline', 'u5_severe_baseline', 
                       'dose3', 't')
  }
  ...
  output <- df %>%
    ...
    # rename(cases = clinical) |>
    mutate_at(vars(n, #n_0_1825, n_91.25_1825,
                   # u5_cases, u5_severe, u5_dalys,
                   clinical, severe,
                   clinper1000vax, sevper1000vax,
                   clinical, cases_lower, cases_upper, 
                   severe,
                   deaths, deaths_lower, deaths_upper,
                   yll:daly_lower),
              sum, na.rm = T) |>  
    select(-age_upper, -age_lower) #, -int_ID, -group

  # separate out baseline scenarios
  none <- output |>
    ungroup() |>
    filter(PEV == 'none' & MDA == 0) |># 
    rename(daly_baseline = daly,
           cases_baseline = clinical,
           severe_baseline = severe,
  ...
  averted <- output |> filter(!(file %in% base_IDs)) |>
    ...
    mutate(dalys_averted = daly_baseline - daly,
           ...
           cases_averted = cases_baseline - clinical,
